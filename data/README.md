...
- Spatial registration between the motor and rail views and the physical time calibration remain to be verified before measurement.
...
| name | records | notes |
| --- | ---: | --- |
| `off_to_on.tif` | 2 channels x 2,089 timepoints | 16-bit ImageJ hyperstack; metadata reports 4,178 images. Channel 0 is DNA rails and channel 1 is motor proteins. |
| `on_to_off.tif` | 2,000 frames at one stored position | 16-bit, 512 x 512 Micro-Manager TIFF. The motor view is the left 256 columns and the rail view is the right 256 columns. Per-frame elapsed-time metadata should be used rather than the nominal interval field. |
